                    ...
                    if (args.length == 0) {
                        ...
                        if (target != null) {
                            if (target.isOnline()) {
                                ...
                                target.sendMessage(ChatColor.DARK_RED + "You have been warned!");
                                target.sendTitle(ChatColor.RED + "You were warned by " + ChatColor.GOLD + sender, "", 20, 60, 20);
                                ...
                                ((Player) sender).playSound(((Player) sender).getLocation(), Sound.ENTITY_VILLAGER_YES, 1f, 1f);
                                target.playSound(target.getLocation(), Sound.ENTITY_GENERIC_EXPLODE, 1f, 1f);
                                Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "say " + ChatColor.DARK_RED + target + ChatColor.RED + " has been warned!");
                            }else {
                ...
                if (args.length == 0) {
                    ...
                    if (target != null) {
                        if (target.isOnline()) {
                            sender.sendMessage(ChatColor.GREEN + "You have succesfully warned " + ChatColor.RED + target);
                            target.sendMessage(ChatColor.DARK_RED + "You have been warned!");
                            target.sendTitle(ChatColor.RED + "You have been warned by " + ChatColor.GOLD + sender, "", 20, 60, 20);
                            target.spawnParticle(Particle.EXPLOSION_HUGE, target.getLocation(), 100);
                            sender.sendMessage(ChatColor.DARK_GREEN + "TIP: " + ChatColor.GREEN + "use /warnlist to see all the commands of warning someone");
                            target.playSound(target.getLocation(), Sound.ENTITY_GENERIC_EXPLODE, 1f, 1f);
                            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "say " + ChatColor.DARK_RED + target + ChatColor.RED + " has been warned!");
